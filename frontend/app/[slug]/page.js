...
function PublicCustomPageContent({ page, inDashboard = false }) {
  return (
    <div className={inDashboard ? "" : "mx-auto max-w-4xl"}>
      <section className="overflow-hidden rounded-[2rem] border border-border bg-card/95 p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur dark:bg-slate-900/70 sm:p-8 lg:p-10">
        <div className="border-b border-border pb-6">
          ...
          {page.shortDescription ? (
            <p className="mt-4 max-w-3xl text-base leading-7 text-slate-500">{page.shortDescription}</p>
          ) : null}
        </div>

        <article className="custom-page-content pt-6" dangerouslySetInnerHTML={{ __html: page.content }} />
      </section>
    </div>
  );
}
